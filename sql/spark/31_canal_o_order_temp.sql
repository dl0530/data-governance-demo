-- 功能：解析Kafka的JSON数据，过滤o_order表，生成结构化临时视图
-- 核心：Kafka的value是Binary类型，需先CAST为STRING再解析JSON
CREATE TEMPORARY VIEW canal_o_order_temp
AS
SELECT
  -- 解析Canal JSON根节点字段
  ...
  get_json_object(CAST(value AS STRING), '$.data[0].total_amount') AS total_amount,
  get_json_object(CAST(value AS STRING), '$.data[0].order_status') AS order_status,
